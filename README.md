...
![image](https://github.com/user-attachments/assets/90d29baa-28b0-4b07-be03-0c6f79a9ea0b)
...
Програма N-Queens має кілька варіантів запуску залежно від потреб користувача:

### Консольна версія:
```bash
python3 nqueens.py console
```
...
### Графічна версія (Tkinter):
```bash
python3 nqueens.py visual
```
У графічній версії користувач може:
1. Вибрати розмір дошки з випадаючого списку (від 4 до 20)
2. Налаштувати швидкість анімації ("Slow", "Medium", "Fast", "Very Fast")
3. Натиснути кнопку "Solve" для початку розв'язування
4. Переглянути процес розміщення ферзів в реальному часі
5. Використовувати кнопки навігації для перегляду різних розв'язків
6. Бачити повну статистику виконання

### Порівняльний аналіз алгоритмів:
```bash
python3 nqueens_comparison.py
```
Запускає бенчмарк, який:
1. Порівнює продуктивність алгоритмів backtracking, BFS та greedy для різних розмірів дошки
2. Генерує графіки часу виконання та кількості кроків
3. Зберігає результати у файл `nqueens_benchmark.png`

### Важливі зауваження:
- Для графічної версії потрібно мати встановлені пакети `tkinter` та `pygame`
- Для бенчмарку потрібен `matplotlib` для генерації графіків
- Для великих дошок (N > 12) BFS може працювати дуже повільно або викликати помилку переповнення пам'яті
- Для консольної візуалізації потрібен пакет `colorama`

Базові вимоги до системи:
- Python 3.6 або новіший
- Залежності: `tkinter`, `pygame`, `matplotlib`, `colorama`

Установка залежностей:
```bash
pip install pygame matplotlib colorama
```
Tkinter зазвичай встановлюється разом з Python, але якщо потрібно:
```bash
# На Ubuntu/Debian:
